...
__all__ = [
    'flatten_pvt_tiny', 'flatten_pvt_small', 'flatten_pvt_medium', 'flatten_pvt_large'
]
...
class FocusedLinearAttention(nn.Layer):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        positional_encoding_parameter = paddle.zeros([1, num_patches // (sr_ratio * sr_ratio), dim])
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            v = F.interpolate(v.transpose([0, 2, 1]), size=x.shape[1], mode='linear').transpose([0, 2, 1])
...
class PyramidVisionTransformer(nn.Layer):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            pos_embed_parameter = paddle.zeros([1, num_patches, embed_dims[i]])
            pos_embed = paddle.create_parameter(shape=pos_embed_parameter.shape, is_bias=True, dtype='float32')
        ...
        for i in range(num_stages):
            ...
            param_init.xavier_uniform(pos_embed)
    ...
    def _get_pos_embed(self, pos_embed, patch_embed, H, W):
        if H * W == self.patch_embed1.num_patches:
            ...
            return F.interpolate(
                pos_embed.reshape([1, patch_embed.H, patch_embed.W, -1]).transpose([0, 3, 1, 2]),
                size=(H, W), mode="bilinear").reshape([1, -1, H * W]).transpose([0, 2, 1])
...
def flatten_pvt_tiny(pretrained=False, **kwargs):
...
def flatten_pvt_small(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def flatten_pvt_medium(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def flatten_pvt_large(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
