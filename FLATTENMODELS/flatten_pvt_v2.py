...
class FocusedLinearAttention(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1 or self.linear:
            v = nn.functional.interpolate(v.transpose([0, 2, 1]), size=x.shape[1], mode='linear').transpose([0, 2, 1])
...
class Block(nn.Layer):
    ...
    def __init__(self, dim, num_patches, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)  # normalized_shape
...
def flatten_pvt_v2_b0(pretrained=False, **kwargs):
...
def flatten_pvt_v2_b1(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
...
def flatten_pvt_v2_b2(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def flatten_pvt_v2_b3(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def flatten_pvt_v2_b4(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def flatten_pvt_v2_b5(pretrained=False, **kwargs):
    model = PyramidVisionTransformerV2(
