...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 2*Wh-1 * 2*Ww-1, nH
        relative_position_bias_table_parameter = paddle.zeros([(2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads])
        self.relative_position_bias_table = paddle.create_parameter(shape=relative_position_bias_table_parameter.shape, is_bias=True, dtype='float32')
        ...
        param_init.xavier_uniform(self.relative_position_bias_table)
...
class FocusedLinearAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        # self.scale = nn.Parameter(paddle.zeros(size=(1, 1, dim)))
        ...
        positional_encoding_parameter = paddle.zeros([1, window_size[0] * window_size[1], dim])
    ...
    def forward(self, x, mask=None):
        ...
        if float(focusing_factor) <= 6:
            ...
            q = (q / q.max(axis=-1, keepdim=True)[0]) ** focusing_factor
            k = (k / k.max(axis=-1, keepdim=True)[0]) ** focusing_factor
...
class FLattenSwinTransformer(nn.Layer):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.ape:
            self.absolute_pos_embed = nn.Parameter(paddle.zeros(1, num_patches, embed_dim))
            absolute_pos_embed = paddle.zeros([1, num_patches, embed_dim])
            self.absolute_pos_embed = paddle.create_parameter(shape=absolute_pos_embed, is_bias=True, dtype='float32')
            param_init.xavier_uniform(self.absolute_pos_embed)
    ...
    def _init_weights(self, layer):
        ...
        #     param_init.kaiming_uniform(layer.weight, nonlinearity="relu")
