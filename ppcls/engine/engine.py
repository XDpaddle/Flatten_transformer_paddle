...
# NEW FOR SMT
from SMTMODEL import build_model_smt
# NEW FOR BIFORMER
from BIFORMERMODELS.models.biformer import create_model
# NEW FOR FLATTENMODELS
from FLATTENMODELS.build import build_model as build_model_flt
...
class Engine(object):
    def __init__(self, config, mode="train"):
        ...
        if config['MODEL']['TYPE'] == 'smt':
            self.model = build_model_smt(config)
        elif config['MODEL']['TYPE'] == 'biformer':
            self.model = create_model(
                config['MODEL']['NAME'],
                pretrained=config['MODEL']['load_release'],
                num_classes=config['MODEL']['nb_classes'],
                drop_rate=config['MODEL']['drop'],
                drop_path_rate=config['MODEL']['drop_path'],
                drop_block_rate=None,
            )
        elif config['MODEL']['type'] == 'flatten_transformer':
            self.model = build_model_flt(config)
        else:
            self.model = build_model(self.config, self.mode)
            # set @to_static for benchmark, skip this by default.
            apply_to_static(self.config, self.model)

        # LOAD SMT PRETRAINED WEIGHT
        if self.config["TRAIN"]["CHECKPOINT_PATH"] is not None:
            checkpoint_smt = paddle.load(self.config["TRAIN"]["CHECKPOINT_PATH"])
            self.model.set_state_dict(checkpoint_smt)
        ...
        if self.mode == "train":
            # std_gpu_num = 8 if isinstance(  # yuan
            #     self.config["Optimizer"],
            #     dict) and self.config["Optimizer"]["name"] == "AdamW" else 4
            std_gpu_num = world_size  # 本机仅单卡，此处为单卡训练做改动
